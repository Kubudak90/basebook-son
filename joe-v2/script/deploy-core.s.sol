...
contract CoreDeployer is Script {
    ...
    function run() public {
        ...
        for (uint256 i = 0; i < chains.length; i++) {
            ...
            // Only add quote assets if factoryV2 exists (not for fresh deployments like Base Sepolia)
            if (deployment.factoryV2 != address(0)) {
                ...
                // For fresh deployments, add WETH as quote asset
                factoryV2_2.addQuoteAsset(IERC20(deployment.wNative));
                console.log("Quote asset whitelisted (WETH) -->", deployment.wNative);
            }
            ...
            // Only transfer ownership if multisig is set
            if (deployment.multisig != address(0)) {
                factoryV2_2.transferOwnership(deployment.multisig);
                console.log("Ownership transferred to multisig -->", deployment.multisig);
            } else {
                console.log("Ownership remains with deployer (no multisig set)");
            }
            vm.stopBroadcast();
        }
    }
    ...
    function _setupBaseSepolia() private {
        StdChains.setChain(
            "base_sepolia",
            StdChains.ChainData({
                name: "Base Sepolia",
